...
def get_data(folder):
    ...
    print(folder + ", average false negatives: " + str(sum(fns) / len(fns)))
...
if __name__ == '__main__':
    ...
    ax.bar(np.arange(len(num_servers))-0.3, lms[0]-fms[0], 0.18, yerr=lss[0], bottom=fms[0], label='Medley, dsm',
    ...
    ax.bar(np.arange(len(num_servers))-0.1, fms[1], 0.18, yerr=fss[1], label='active, dfdt', 
        ...
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    ax.bar(np.arange(len(num_servers))-0.1, lms[1]-fms[1], 0.18, yerr=lss[1], bottom=fms[1], label='active, dsm',
    ...
    ax.bar(np.arange(len(num_servers))+0.1, fms[2], 0.18, yerr=fss[2], label='passive, dfdt', 
        ...
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    ax.bar(np.arange(len(num_servers))+0.1, lms[2]-fms[2], 0.18, yerr=lss[2], bottom=fms[2], label='passive, dsm',
    ...
    ax.bar(np.arange(len(num_servers))+0.3, fms[3], 0.18, yerr=fss[3], label='act & pas, dfdt', 
        ...
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    ax.bar(np.arange(len(num_servers))+0.3, lms[3]-fms[3], 0.18, yerr=lss[3], bottom=fms[3], label='act & pas, dsm',
