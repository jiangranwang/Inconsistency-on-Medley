...
def get_data(folder, num_server):
    ...
    avgFalseNegative = sum(fns) / len(fns) * 100.0 / num_server
...
if __name__ == '__main__':
    ...
    fig, (ax, ax2) = plt.subplots(nrows=2, gridspec_kw={'height_ratios': [4, 1]})
    # fig, ax = plt.subplots()
    ax.grid()
    ...
    ax.set_xticklabels(num_servers)
    ax.set_ylim(0, 350)
    # ax.set_yticks([0, 100, 200, 300])
    ax.legend(loc='upper left', ncol=2, bbox_to_anchor=(-0.005, 1.03), fontsize=10)


    ax2.bar(np.arange(len(num_servers))-0.3, fns[0], 0.18, label='Medley', 
        color=ps.get_bar_color('naive'), hatch=ps.get_pattern('naive', 2),
        edgecolor=ps.get_edge_color('naive'),
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    ax2.bar(np.arange(len(num_servers))-0.1, fns[1], 0.18, label='active', 
        color=ps.get_bar_color('active'), hatch=ps.get_pattern('active', 2),
        edgecolor=ps.get_edge_color('active'),
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    ax2.bar(np.arange(len(num_servers))+0.1, fns[2], 0.18, label='passive', 
        color=ps.get_bar_color('passive'), hatch=ps.get_pattern('passive', 2),
        edgecolor=ps.get_edge_color('passive'),
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    ax2.bar(np.arange(len(num_servers))+0.3, fns[3], 0.18, label='act & pas', 
        color=ps.get_bar_color('act_pas'), hatch=ps.get_pattern('act_pas', 2),
        edgecolor=ps.get_edge_color('act_pas'),
        error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))

    ax2.yaxis.tick_right()
    ax2.yaxis.set_label_position("right")
    ax2.set_ylabel('Incomplete\nDissemination (%)', rotation=-90, labelpad=30)
    ax2.set_xlabel(r'Number of servers $m$')
    ax2.set_ylim(0.5, 0)
    ax2.set_xticks(np.arange(len(num_servers)))
    ax2.set_xticklabels(num_servers)
    ax2.legend(ncol=2, fontsize=8)

    fig.set_size_inches(6, 3)
    plt.subplots_adjust(hspace=0, bottom=0.15, right=0.86, top=0.95)
